...
summary: Cree un nuevo Microsoft SQL Server dentro de NetApp Workload Factory para bases de datos. 
---
= Crea un Microsoft SQL Server en NetApp Workload Factory for Databases
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
Para obtener más información, consulte link:https://aws.amazon.com/blogs/modernizing-with-aws/sql-server-high-availability-amazon-fsx-for-netapp-ontap/["Implementaciones de alta disponibilidad de SQL Server con Amazon FSx for NetApp ONTAP"^] .
...
Usted debe link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["otorgar permisos de creación de host de base de datos"^] en su cuenta de AWS para crear un nuevo host de base de datos en Workload Factory.
...
Puede utilizar los modos de implementación _Creación rápida_ o _Creación avanzada_ para completar esta tarea en Workload Factory con permisos del modo _Automatizar_.  También puede utilizar las siguientes herramientas disponibles en Codebox: API REST, AWS CLI, AWS CloudFormation y Terraform. link:https://docs.netapp.com/us-en/workload-setup-admin/use-codebox.html#how-to-use-codebox["Aprende a usar CodeBox para la automatización"^] .
...
. En el mosaico Bases de datos, seleccione *Implementar host* y luego seleccione *Microsoft SQL Server* en el menú.
...
+
Las credenciales de AWS con permisos de _solo lectura_ permiten que Workload Factory genere una plantilla de CloudFormation para que usted la use en la consola de AWS CloudFormation.
...
+
Para las cuentas GovCloud, introduce el valor del parámetro ARN de AWS SSM en el siguiente formato:
`{fsx:{"username":"fsxadmin","password":"pass"}}`.

+
El valor se proporciona en el Parameter Store bajo Parameter details en la AWS Management Console. El tipo de parámetro AWS SSM debe ser "Secure String" con el nombre en el formato `/netapp/wlmdb/<anyname>`. Obtén más información sobre cómo recuperar parámetros SSM desde link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].
...
+
Para las cuentas GovCloud, introduce el valor ARN del parámetro SSM de AWS para las credenciales de Active Directory en el siguiente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. El tipo de parámetro SSM de AWS debe ser "Secure String" con el nombre en el formato `/netapp/wlmdb/<anyname>`. Obtén más información sobre cómo recuperar parámetros SSM de link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].
...
+
Para las cuentas GovCloud, introduce el valor ARN del parámetro AWS SSM para las credenciales de FSx para ONTAP en el siguiente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. El tipo de parámetro AWS SSM debe ser "Secure String" con el nombre en el formato `/netapp/wlmdb/<anyname>`. Obtén más información sobre cómo recuperar parámetros SSM desde link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].

+
Un nuevo sistema de archivos FSX para ONTAP puede agregar 30 minutos o más de tiempo de instalación.

... *Selecciona un FSx for ONTAP* existente: selecciona el nombre de FSx for ONTAP e introduce un nombre de usuario y una contraseña para el sistema de archivos.
+
...
--
.Pasos
. Inicie sesión utilizando uno de loslink:https://docs.netapp.com/us-en/workload-setup-admin/console-experiences.html["experiencias de consola"^] .  En el mosaico Bases de datos, seleccione *Implementar host* y luego seleccione *Microsoft SQL Server* en el menú.
...
+
Si desea completar el formulario de creación de nuevo servidor en Workload Factory para poder descargar una plantilla de archivo YAML completa para su implementación en AWS CloudFormation, siga la *Opción 2* para asegurarse de tener los permisos necesarios para crear el nuevo servidor dentro de AWS CloudFormation.  Agregue manualmente las credenciales y los permisos necesarios para el modo _solo lectura_ para las cargas de trabajo de la base de datos.
...
.. *Zonas de disponibilidad*: seleccione zonas de disponibilidad y subredes según el modelo de implementación que haya seleccionado.  Las subredes no deben compartir la misma tabla de rutas para lograr una alta disponibilidad.
+
...
.. *Grupo de seguridad*: Se crea un grupo de seguridad de carga de trabajo para permitir los puertos y protocolos necesarios para la comunicación en clúster de Microsoft SQL y Windows en los nodos. Además, puedes seleccionar uno o más grupos de seguridad.
...
... Introduce el nombre de usuario y la contraseña del dominio de Active Directory.
+
Para las cuentas GovCloud, introduce el valor ARN del parámetro SSM de AWS para las credenciales de Active Directory en el siguiente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. El tipo de parámetro SSM de AWS debe ser "Secure String" con el nombre en el formato `/netapp/wlmdb/<anyname>`. Obtén más información sobre cómo recuperar parámetros SSM de link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].

... *Controlador de dominio preferido*: de manera opcional, ingrese el controlador de dominio preferido que se utilizará para unirse a Active Directory.
... *Ruta de unidad organizativa preferida*: de manera opcional, ingrese la unidad organizativa (OU) preferida en Active Directory a la que desea unirse.
... *Grupo de Active Directory de destino*: de manera opcional, ingrese el grupo de Active Directory de destino al que se agregarán las computadoras.
...
... * Crear nuevo FSX para ONTAP*: Introduzca el nombre de usuario y la contraseña.
+
Para las cuentas GovCloud, introduce el valor ARN del parámetro AWS SSM para las credenciales FSx para ONTAP en el siguiente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. El tipo de parámetro AWS SSM debe ser "Secure String" con el nombre en el formato `/netapp/wlmdb/<anyname>`. Obtén más información sobre cómo recuperar parámetros SSM de link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].
...
.El futuro
Ahora puedeslink:create-database.html["crear una base de datos en Workload Factory for Databases"] .

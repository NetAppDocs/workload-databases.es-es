...
---
= Crea un servidor PostgreSQL en NetApp Workload Factory for Databases
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
.. *Grupo de seguridad*: Se crea un grupo de seguridad de carga de trabajo para permitir los puertos y protocolos necesarios para PostgreSQL. Además, puedes seleccionar uno o varios grupos de seguridad.
...
+
Para las cuentas GovCloud, introduce el valor ARN del parámetro SSM de AWS para las credenciales en el siguiente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. El tipo de parámetro SSM de AWS debe ser "Secure String" con el nombre en el formato `/netapp/wlmdb/<anyname>`. Obtén más información sobre cómo recuperar parámetros SSM desde link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].
...
.. *Servicio de notificación simple*: Opcionalmente, puedes habilitar el Servicio de notificación simple (SNS) para esta configuración seleccionando un tema SNS para PostgreSQL en el menú desplegable.
+

...
... * Crear nuevo FSX para ONTAP*: Introduzca el nombre de usuario y la contraseña.
+
+ Para las cuentas GovCloud, introduce el valor ARN del parámetro AWS SSM para las credenciales de FSx for ONTAP en el siguiente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. El tipo de parámetro AWS SSM debe ser "Secure String" con el nombre en el formato `/netapp/wlmdb/<anyname>`. Aprende más sobre cómo recuperar parámetros SSM desde link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].

+
+ Un nuevo sistema de archivos FSx para ONTAP puede añadir 30 minutos o más de tiempo de instalación.

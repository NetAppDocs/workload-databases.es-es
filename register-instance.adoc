...
Registrar una instancia (SQL Server) o una base de datos (Oracle) implica tres pasos: autenticación de la instancia o base de datos, autenticación de FSx for ONTAP y preparación. Para prepararte, instala los módulos de AWS, NetApp y PowerShell en la instancia o base de datos, y cumple los requisitos mínimos para las funciones de Workload Factory for Databases como link:analyze-error-logs.html["análisis de registros de errores"] o link:optimize-configurations.html["evaluación bien diseñada"].
...
Los usuarios de AWS GovCloud (US) deben utilizar AWS Secrets Manager para autenticar las credenciales de FSx para ONTAP. Para el resto de usuarios, debes autenticar FSx para ONTAP con las credenciales del sistema de archivos.
...
.. Introduce el ARN de parámetros de AWS SSM con las credenciales de FSx para ONTAP para las cuentas de GovCloud o el nombre de usuario y la contraseña del sistema de archivos de FSx para ONTAP para el resto de cuentas y luego selecciona *Next*.
+
Para las cuentas GovCloud, el valor ARN del parámetro SSM de AWS se proporciona en el Almacén de Parámetros bajo detalles del parámetro en la consola de gestión de AWS en el siguiente formato: `{fsx:{"username":"fsxadmin","password":"pass"}}`. El tipo de parámetro SSM de AWS debe ser "Cadena segura" con el nombre en el formato `/netapp/wlmdb/<fsx_filesystemID>`. Más información sobre cómo recuperar parámetros SSM de link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].
...
.. Introduce el ARN de parámetros de AWS SSM con las credenciales de FSx para ONTAP para las cuentas de GovCloud o el nombre de usuario y la contraseña del sistema de archivos de FSx para ONTAP para el resto de cuentas y luego selecciona *Next*.
+
El tipo de parámetro AWS SSM debe ser "Secure String" con el nombre en el formato `/netapp/wlmdb/<fsx_filesystemID>`. Introduce el valor para el ARN del parámetro AWS SSM en el siguiente formato:
`{fsx:{"username":"fsxadmin","password":"pass"}}`. Obtén más información sobre cómo recuperar parámetros SSM desde link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud documentación"^].

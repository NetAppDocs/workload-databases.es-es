...
* *Monitorización continua*: Registra recursos para habilitar la monitorización continua. Workload Factory analiza tu entorno de bases de datos con regularidad, ofrece recomendaciones continuas para optimizar tus configuraciones y soluciona los problemas automáticamente.
...
Para la evaluación única, solo tienes que proporcionar las credenciales una vez para las instancias de Microsoft SQL Server o la base de datos Oracle que quieras analizar.

Para la monitorización continua, necesitas registrar tus recursos, asociar un enlace y conceder permisos en tu cuenta de AWS.
...
* *Replicación interregional (CRR)*
+
Habilita la replicación entre regiones (CRR) para mejorar la disponibilidad de los datos y las capacidades de recuperación ante desastres de tus bases de datos Oracle. CRR ayuda a proteger frente a las interrupciones regionales y garantiza la continuidad del negocio replicando los datos en ubicaciones geográficamente dispersas.

* *Instantáneas consistentes con las aplicaciones*
+
Usa instantáneas consistentes con las aplicaciones con NetApp SnapCenter para tomar instantáneas precisas y fiables de tus datos de volumen en un momento específico. Esto mantiene tus aplicaciones funcionando sin problemas y tus datos seguros. SnapCenter facilita las copias de seguridad y te ayuda a restaurar los datos de forma rápida y correcta, reduciendo el tiempo de inactividad y protegiendo tus cargas de trabajo más importantes.

...
. Seleccione el menú image:workloads-menu-icon.png["El ícono del menú de hamburguesa se utiliza para navegar a cargas de trabajo como almacenamiento, eda, ai, bases de datos, vmware y administración."] y luego seleccione *Bases de datos*.
. En el menú Bases de datos, selecciona *Explorar ahorros*.
. En Explorar ahorros, selecciona la pestaña *SQL Server on-premises*.
. En la pestaña local de SQL Server, descargue el script para evaluar sus entornos de SQL Server locales.
+
.. Descarga el script de evaluación. El script es una herramienta de recopilación de datos basada en PowerShell. Recopila y luego sube los datos de configuración y rendimiento de SQL Server a Workload Factory. El asesor de migración evalúa los datos y planifica la implementación de FSx para ONTAP para tu entorno de SQL Server.
+
